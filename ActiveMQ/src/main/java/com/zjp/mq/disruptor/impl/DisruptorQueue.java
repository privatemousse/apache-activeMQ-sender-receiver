...
/**
 * @author lsp copyright all reserved 基于disruptor的队列
 */
public class DisruptorQueue implements Disruptor, InitializingBean {
	...
	public void publish(QMessage message) {
		if (message != null) {
			log.info("add message to disruptor queue...start");
			...
			log.info("add message to disruptor queue... end");
		}
	}
	...
	public void afterPropertiesSet() throws Exception {
		Preconditions.checkState(bufferSize > 0 && thread > 0 && consume > 0,
				"bufferSize and thread must great than zero...");
		...
		for (int i = 0; i < consume; i++) {
			handles[i] = EventWorkHandle.builder().brokerConfig(brokerConfig).producerCache(producerCache)
